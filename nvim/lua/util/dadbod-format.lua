...
-- Overlay every vertical │ separator on a line with the dim DboutBorder group.
-- Used for data rows and the header row, whose cells are coloured separately.
local function dim_vertical_separators(bufnr, lnum, line)
  local vpos = 1
  while true do
    local vfound = line:find(box.v, vpos, true)
    if not vfound then
      break
    end
    vim.api.nvim_buf_set_extmark(bufnr, ns_id, lnum, vfound - 1, {
      end_col = vfound - 1 + #box.v,
...
local function displaywidth(str)
  -- Fast path: printable ASCII only (0x20-0x7E), where byte length == display
  -- width. Anything else — high bytes (multi-byte UTF-8) OR control characters
  -- such as a mid-line \r (which renders as ^M = 2 cells) — needs
  -- strdisplaywidth for an accurate count, or the bordered table misaligns.
  if not str:find("[^\x20-\x7e]") then
    return #str
  end
...
local function try_parse_with_delimiter(lines, delim)
  ...
  for _, line in ipairs(lines) do
    ...
    if
      ...
      or line:match("%[Note%]")
      or line:match("%[Error%]")
      or line:match("^%a[%w%s]* is \"") -- psql \pset echo, e.g.: Null display is "NULL".
    then
      goto continue
    end
    -- Skip empty lines
...
local function parse_fixed_width(lines)
  ...
  for i = separator_idx + 1, #lines do
    ...
    if line:match("^%s*%(?%d+ rows? affected%)") or line:match("^%s*%(%d+%s+rows?%)%s*$") then
      break
    end
...
local function split_result_sets(lines)
  ...
  for _, line in ipairs(lines) do
    ...
    if
      ...
      or line:match("^%s*%(%d+%s+rows?%)%s*$") -- psql footer: (2 rows)
    then
    ...
    -- Handle psql DML command tags: UPDATE 3 / DELETE 2 / INSERT 0 1 (count is the last number)
    local pg_dml_count = line:match("^UPDATE%s+(%d+)%s*$")
      or line:match("^DELETE%s+(%d+)%s*$")
      or line:match("^INSERT%s+%d+%s+(%d+)%s*$")
    if pg_dml_count then
      if #current_set > 0 then
        result_sets[#result_sets + 1] = { lines = current_set, status = current_status }
        current_set = {}
      end
      local word = pg_dml_count == "1" and "row" or "rows"
      result_sets[#result_sets + 1] = { lines = {}, status = "(" .. pg_dml_count .. " " .. word .. " affected)" }
      current_status = ""
      in_data = false
      goto continue
    end
...
local function detect_column_type(_, rows, col_idx)
  ...
  for _, row in ipairs(rows) do
    ...
    -- Timestamp pattern: datetime (YYYY-MM-DD HH:MM:SS / YYYY-MM-DDTHH:MM:SS)
    -- or date-only (YYYY-MM-DD). Both are coloured as timestamps.
    if val:match("^%d%d%d%d%-%d%d%-%d%d[T ]%d%d:%d%d:%d%d") or val:match("^%d%d%d%d%-%d%d%-%d%d$") then
...
local function analyze_columns(headers, rows)
  ...
  -- When set, render every column at its full width (no caps / no ellipsis).
  local no_trunc = vim.b.dbout_no_truncate == 1
  ...
  for i = 1, #headers do
    ...
    local data_width = no_trunc and actual_max or math.min(actual_max, cap)
...
local function apply_highlighting()
  ...
  for i, line in ipairs(lines) do
    ...
    -- Header rows: │...│ line following a ┌...┐ line. Bold header text at a
    -- low priority so the dim │ separators (default priority) override it.
    if i >= 2 and lines[i - 1]:find("^\xe2\x94\x8c") and line:find("^\xe2\x94\x82") then
      ...
      dim_vertical_separators(bufnr, lnum, line)
      goto continue_line
    end
  ...
  for table_idx = 1, #col_info_list do
    ...
    for lnum_1 = data_start, data_end do
      ...
      -- Dim the vertical │ separators in this data row (cell values are
      -- highlighted separately below; the bars would otherwise be default fg).
      dim_vertical_separators(bufnr, lnum, line)
...
local function open_expand_window(value, header)
  -- Remember the dbout window so we can hand focus back to it on close — a
  -- plain :close lands on the DBUI drawer instead. Snapshot window sizes too,
  -- so closing the popup doesn't let 'equalalways' resize the dbout split.
  ...
  -- Close the popup, restore the pre-popup window sizes, and return focus to
  -- the dbout window it was opened from.
  local function close_and_return()
    vim.cmd("close")
    pcall(vim.cmd, restore_sizes)
...
local frozen = {
  ...
  buf = nil,
  augroup = nil,
  dbout_buf = nil, -- the dbout buffer this header tracks (may be unfocused)
  last_topline = -1,
  last_leftcol = -1,
  last_width = -1,
}
...
local function apply_frozen_highlights(bufnr, num_lines)
  ...
  for i = 0, math.min(num_lines - 1, 3) do
    ...
    if line and group then
      -- Header line: low priority so its dim │ separators override the bold.
      vim.api.nvim_buf_set_extmark(bufnr, ns_id, i, 0, {
        end_col = #line,
        hl_group = group,
        priority = group == "DboutHeader" and 1 or nil,
      })
      if group == "DboutHeader" then
        dim_vertical_separators(bufnr, i, line)
      end
    end
  end
end

-- Locate the window (in this tabpage) showing the tracked dbout buffer.
-- Prefer the current window so cursor/scroll reads stay cheap, but fall back
-- to any split displaying it so the header survives focus moving away.
local function find_dbout_win()
  local b = frozen.dbout_buf
  if not b or not vim.api.nvim_buf_is_valid(b) then
    return nil
  end
  local cur = vim.api.nvim_get_current_win()
  if vim.api.nvim_win_get_buf(cur) == b then
    return cur
  end
...
local frozen_updating = false

local function do_update_frozen_header()
  local win = find_dbout_win()
  if not win then
    close_frozen_header()
    return
  end
  local bufnr = frozen.dbout_buf
  if vim.b[bufnr].dbout_is_formatted ~= 1 then
    close_frozen_header()
    return
  end

  -- Read scroll position from the dbout window itself (it may be unfocused).
  local topline, leftcol
  ...
  local width_changed = width ~= frozen.last_width

  if not topline_changed and not leftcol_changed and not width_changed then
    return
  end

  -- Horizontal-only scroll: just sync the float's scroll position
  if not topline_changed and not width_changed and leftcol_changed then
...
-- Re-entrancy guard: reconfiguring/creating the float can itself emit
-- WinScrolled, which would otherwise recurse back into here.
local function update_frozen_header()
  if frozen_updating then
    return
  end
  frozen_updating = true
  pcall(do_update_frozen_header)
  frozen_updating = false
end

function M.setup_frozen_headers(bufnr)
  frozen.dbout_buf = bufnr
  ...
  -- Check on cursor movement within the dbout buffer (cached early-exit)
  ...
  -- Scroll/resize of any window. update_frozen_header locates the dbout window
  -- itself and only rebuilds when its topline/leftcol/width actually changed,
  -- so scrolling an unrelated split (e.g. the query editor) is a cheap no-op.
  vim.api.nvim_create_autocmd("WinScrolled", {
    group = frozen.augroup,
    callback = update_frozen_header,
  })

  -- Re-evaluate when focus moves between windows or buffers, so the header
  -- stays anchored to the dbout split even while editing the query elsewhere
  -- (and is dropped when the dbout window is no longer visible).
...
--- Called after a query completes (DBExecutePost). A 0-row result writes no
--- output at all, leaving the dbout window blank — replace that with a status
--- line so the result is unambiguous. No-op for buffers that have content
--- (the normal formatter handles those) or were already finalized.
function M.finalize_empty()
  for _, buf in ipairs(vim.api.nvim_list_bufs()) do
    if
      vim.api.nvim_buf_is_loaded(buf)
      and vim.bo[buf].filetype == "dbout"
      and vim.b[buf].dbout_is_formatted ~= 1
    then
      ...
      if not has_content then
        local msg = "(0 rows affected)"
        vim.bo[buf].modifiable = true
        vim.api.nvim_buf_set_lines(buf, 0, -1, false, { msg })
        vim.bo[buf].modifiable = false
        vim.bo[buf].modified = false
        vim.b[buf].dbout_is_formatted = 1
        pcall(vim.api.nvim_buf_set_extmark, buf, ns_id, 0, 0, { end_col = #msg, hl_group = "DboutRowCount" })
      end
    end
  end
end
...
function M.auto_format()
  ...
  poll_timer:start(
    ...
    vim.schedule_wrap(function()
      ...
      -- Already finalized (formatted normally, or marked as an empty result by
      -- the DBExecutePost handler) — stop polling so an empty result doesn't
      -- spin until the timeout.
      if vim.b[dbout_bufnr].dbout_is_formatted == 1 then
        poll_timer:stop()
        poll_timer:close()
        poll_timer = nil
        return
...
-- Dim "meta" lines — errors and status/row-count lines — in an otherwise
-- unformatted dbout buffer (a lone "Error: ..." or "(0 rows affected)" that
-- produced no table would otherwise render in the default fg / white).
-- force_hl: when set (e.g. "DboutError"), every non-empty line gets that group
-- — used for multi-line error output (SQL Server errors span several lines, so
-- pattern-matching per line would leave the detail lines default-white).
local function highlight_meta(bufnr, force_hl)
  vim.api.nvim_buf_clear_namespace(bufnr, ns_id, 0, -1)
  for i, line in ipairs(vim.api.nvim_buf_get_lines(bufnr, 0, -1, false)) do
    if #line > 0 then
      ...
      if not hl then
        if
          line:match("^%s*[Ee]rror") or line:match("^%s*[Mm]sg%s") or line:match("^%s*ERROR")
          or line:match("^%s*failed") or line:match("^%s*ORA%-") or line:match("^%s*PLS%-")
          or line:match("^%s*SP2%-") or line:match("^%s*SQL Error")
        then
          hl = "DboutError"
        elseif line:match("^%s*%[.+%]%s+%(") or line:match("^%s*%(?%d+%s+rows?") or line:match("^%s*Query OK") then
          hl = "DboutRowCount"
        end
      end
      if hl then
        pcall(vim.api.nvim_buf_set_extmark, bufnr, ns_id, i - 1, 0, { end_col = #line, hl_group = hl })
      end
    end
  end
end
...
function M.format()
  ...
  -- Error output: dim it (instead of leaving it default-white) and stop.
  local content = table.concat(lines, "\n")
  local is_error = first_line:match("^[Mm]sg%s") or first_line:match("^ERROR") or first_line:match("^[Ee]rror:")
    or first_line:match("^failed") or first_line:match("^ORA%-") or first_line:match("^PLS%-")
    or first_line:match("^SP2%-") or first_line:match("^SQL Error")
    or content:match("Msg%s+%d+.*Level%s+%d+")
    or content:match("ERROR%s+%d+%s+%(%d+%)")
    or content:lower():find("incorrect syntax")
  ...
  if is_error then
    highlight_meta(0, "DboutError") -- dim the whole (possibly multi-line) error
  ...
  for _, result_set in ipairs(result_sets) do
    ...
    -- Separate from any previous output (avoid stacking blank lines)
    if #all_formatted > 0 and all_formatted[#all_formatted] ~= "" then
      all_formatted[#all_formatted + 1] = ""
    end

    -- Build the row count caption (rendered below the table)
    ...
    local caption = tbl_name ~= "" and ("[" .. tbl_name .. "] " .. count_text) or count_text
    table_name_idx = table_name_idx + 1

    -- The table starts on the next line (no caption above it anymore).
    ...
    -- Table first, then the caption beneath it, then a blank for breathing room.
    vim.list_extend(all_formatted, formatted)
    all_formatted[#all_formatted + 1] = caption
    all_formatted[#all_formatted + 1] = ""
  ...
  if #all_formatted == 0 then
    -- No table produced (e.g. a lone "(0 rows affected)" status line from an
    -- empty SELECT). Dim the meta lines rather than leaving them default-white.
    highlight_meta(0)
    vim.b.dbout_is_formatted = 1
    return
  end
...
--- Toggle inline truncation: re-render the bordered table with column widths
--- either capped (default, with … ellipsis) or at full width (no truncation).
--- Row positions are unchanged, so the cursor line is preserved.
function M.toggle_truncate()
  ...
  -- Flip the per-buffer flag
  vim.b.dbout_no_truncate = (vim.b.dbout_no_truncate == 1) and 0 or 1

  -- Remember where we were (column counts change, line counts don't)
  local cursor = vim.api.nvim_win_get_cursor(0)

  -- Re-render from the stored raw output under the new flag
  vim.bo.modifiable = true
  vim.api.nvim_buf_set_lines(0, 0, -1, false, raw_content)
  vim.bo.modifiable = false
  vim.b.dbout_is_formatted = 0
  ...
  vim.notify(vim.b.dbout_no_truncate == 1 and "Showing full values" or "Showing truncated values")
end
