...
local function render_detail()
  ...
  if state.additional_expanded then
    ...
    if #additional == 0 then
      ...
      -- Append a (possibly multi-line) field value as one or more buffer lines.
      -- nvim_buf_set_lines rejects list items containing newlines, so every
      -- value is split; continuation lines align under the label. Each produced
      -- line maps back to this entry/field so cursor-based editing still works.
      local function add(prefix, value, idx, field)
        local segs = vim.split(tostring(value or ""), "\n", { plain = true })
        if #segs == 0 then
          segs = { "" }
        end
        local indent = string.rep(" ", #prefix)
        for i, seg in ipairs(segs) do
          lines[#lines + 1] = (i == 1 and prefix or indent) .. (seg:gsub("\r", ""))
          state.additional_entry_lines[#lines] = idx - 1
          state.additional_field_lines[#lines] = field
        end
      end

      for idx, entry in ipairs(additional) do
        lines[#lines + 1] = ""
        add("  " .. idx .. ". ", entry.name or "(unnamed)", idx, "name")
        add("     url:  ", entry.url or "", idx, "url")
        add("     user: ", entry.username or "", idx, "username")
        ...
        add("     pass: ", apwd_display, idx, "password")

        if (entry.extra or "") ~= "" then
          -- clean_html + split, matching the main notes section's treatment.
          add("     note: ", clean_html(entry.extra), idx, "extra")
        end
      end
    end
  else
